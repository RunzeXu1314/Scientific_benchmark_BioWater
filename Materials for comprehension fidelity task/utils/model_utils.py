...
def load_model_and_tokenizer(model_fpath, tokenizer_only=False):
    ...
    # ---------- 新增：Ollama 模型 ----------
    if model_fpath.startswith("ollama/"):
        model_name = model_fpath.replace("ollama/", "")
        print(f"Using Ollama model: {model_name}")
        # 返回一个自定义对象，type 标记为 'ollama'
        model = {"type": "ollama", "name": model_name}
        tokenizer = None   # Ollama 不需要 tokenizer
